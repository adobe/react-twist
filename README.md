...
By default, React only re-renders a component when its props or state changes, so even if your data model uses observables, it doesn't know when to re-render. But that's easy to fix - the `@Component` decorator enhances `React.Component`, so that whenever any observables used in the `render()` function changes, it'll trigger a re-render. This is automatic - you don't need to tell React when to update!
...
> Note: `undeclaredAttributes()` was previous called `childAttributes()` - the old API is still supported for now, but is deprecated.
...
Let's say the `render()` function of your component depends on many different observables, and these change all at once. If we're not careful, and trigger a re-render on every change, we may end up re-rendering the component multiple times, even though there was only a single "logical" change. Similarly, if you make lots of changes in a short period of time, you could end up re-rendering more often than necessary.
...
One of the biggest performance issues with React is the granularity of updates. If a component has a really complex `render()` function, you can end up with performance issues - especially if doing a lot of iteration over arrays (e.g. via `<repeat>`).
...
To use this module, add `@twist/react` and `@twist/core` to your `package.json`. If you're using webpack, you then need to add the following to your webpack configuration (`webpack.conf.js`):

> TODO: Update once we have the twist webpack module.
